...
def break_into_words_and_generate_tokens(string):
    tokens = []
    str_len = len(string)
    line_number = 1
    char_index = 0
    ...
    while char_index < str_len:
        ...
        char_index = char_index if (char_index == str_len) else char_index + 1
        ...
        # ! punctuators
            ...
            # ? checking for double colon | used for extend
            elif (temp == ":" and string[char_index] == ":"):
                tokens.append(generate_token(temp + ":", line_number))
                char_index += 1
            # ? check for 2D array declaration
            elif (temp == "[" and string[char_index] == "]" and string[char_index + 1] == "[" and string[char_index + 2] == "]"):
                char_index += 3
                if (word != ""):
                    tokens.append(generate_token(word, line_number))
                    word = ""
                tokens.append(generate_token(temp + "][]", line_number))
            # ? check for 1D array declaration
            elif (temp == "[" and string[char_index] == "]"):
                char_index += 1
                if (word != ""):
                    tokens.append(generate_token(word, line_number))
